...
const CATEGORY_DEFAULTS: Partial<Record<ProductCategory, { defaultUsageAmount: string }>> = {
  BOTOX: { defaultUsageAmount: "20" },
  FILLER: { defaultUsageAmount: "1" },
};
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      try {
        ...
        setErrors({ _server: data.error ?? "Failed to save product." });
      } catch {
        setErrors({ _server: "Failed to save product." });
      }
    }
  }

  const outerUnit = UNIT_LABELS[form.unitType] ?? form.unitType;
